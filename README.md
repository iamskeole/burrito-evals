...
- **`data/gpt_oss/`, `data/bfcl_v4/`** — the raw per-run model outputs including full reasoning traces (one directory per backend×seed configuration; the BFCL side also carries the v4 dataset and expected outputs). The `__discard_*` directories hold runs that were excluded from every reported number (bot-walled browser hits, a wrong-effort batch) and are kept for transparency.
...
- **`report/`** — `report.md` (the full report, seven findings), `post.md` (the public write-up), `story.md` (the reasoning-effort deep dive), `plots.md` (the canonical record of all 44 figures), plus `sections/` (the report split per section) and planning notes.
...
- **Benchmarks** — BFCL v4 in three modes: non-live (simple python/java/javascript, irrelevance), live (live relevance/simple, with a real browser), and `multi_turn_base` (agentic, 4 turns); plus AIME25 and GPQA from OpenAI's GPT-OSS reasoning suite.

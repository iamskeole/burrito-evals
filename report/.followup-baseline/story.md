...
Low effort starts at 100% at 181 tokens where the model answers quickly and correctly on the easy problems it can solve. Then it crashes to 0% as reasoning token count increases. The model is forced to think longer than its effort level supports. It wanders and degrades.
...
Median token count increases exponentially from low to medium to high. On AIME25, low uses 2,169 output tokens. Medium uses 11,816. High uses 38,971. The reasoning-to-output token ratio shifts from 70% at low effort to 98% at high effort. The model devotes increasingly more of its output to internal reasoning, but accuracy gains do not scale with that investment.
...
*This story was extracted from a broader evaluation covering 320K runs, 300K questions, and 1,062 GPU hours of compute. The full report covers seven findings including benchmark design flaws, template mismatches, and wire API differences. The reasoning story is the most transferable lesson and is presented here as a standalone analysis.*
